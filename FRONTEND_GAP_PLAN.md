...
- [ ] Add `firebase` SDK; `src/lib/firebase.ts` init from `app.config` / EAS env (`EXPO_PUBLIC_FIREBASE_*`). User supplies the **Web SDK** config (public keys) for project `kazi-manufacturing`. (The `key.json` service-account key is for read-only schema inspection only — never ship it in the app; see Section 0.)
...
Build:
- [ ] `src/auth/` → real Firebase Auth (`signInWithEmailAndPassword`, `sendPasswordResetEmail`, `onAuthStateChanged`). Session token in `expo-secure-store`, not AsyncStorage. _(waits on Firebase config — Track B. `mock-auth.ts` extended in the meantime: derives `appRole` from the email persona, carries `permissions` overrides, `setDevRole()` for testing.)_
...
- [x] CSV — `src/lib/export/csv.ts` (`toCSV`, no deps), used by Billing's list export (item 15); Finance ledger + Attendance exports still to wire (items 18-note / 27).
- [ ] `src/lib/export/csv.ts` — Billing list export, Finance ledger export, Attendance report export (all currently absent).
- [ ] Wire every toast-simulated "download / email / share / export" to a real share sheet.
...
- [ ] **Edit** — many mobile screens can create + delete but not edit an existing record (Purchases, Finance expenses, Sales orders, Tasks in some cases). Reference edits everything inline or via a sheet.
...
- [ ] **Permission notices** — read-only banner when `!can(section)`.
...
22. **Order Management** (4.1) — the 7-stage kanban CRUD; reconcile `orders` ownership with Production + Sales _(needs design)_.
...
25. **Sales — rebuild as pipeline overview** from real orders (3.9).
...
33. **Bug Report** (4.3).
34. **Changelog — GitHub commits feed** or confirm static (3.17).
...
3. **Sales vs Order Management vs Production orders** — which screen owns `orders` CRUD? Recommend: Order Management owns CRUD, Production consumes + logs output, Sales is a read-only overview.
