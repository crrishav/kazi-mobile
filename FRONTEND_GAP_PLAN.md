...
- [x] Add `firebase` SDK; `src/lib/firebase.ts` init from `app.config` / EAS env (`EXPO_PUBLIC_FIREBASE_*`). **Done 2026-08-30** — `firebase@^12.18.0` installed; `src/lib/firebase.ts` is an inert lazy scaffold (`isFirebaseConfigured` + `getDb`/`getFirebaseAuth`, throw until env vars set). Still needs the user's **Web SDK** config (public keys) for project `kazi-manufacturing` to go live. (The `key.json` service-account key is for read-only schema inspection only — never ship it in the app; see Section 0.)
